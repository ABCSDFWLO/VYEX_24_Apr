...
func _on_login_button_pressed() -> void:
	var url := "http://"+Constants.HOST+Constants.PORT_WITH_COLON+Constants.LOGIN_URL
	var body := JSON.new().stringify({"email":login_id_lineedit.text,"password":login_password_lineedit.text})
	var error := login_http_request.request(url,[],HTTPClient.METHOD_POST, body)
	if error != OK:
...
func _on_submit_button_pressed() -> void:
	...
	if (password == password_check):
		var url := "http://"+Constants.HOST+Constants.PORT_WITH_COLON+Constants.REGISTER_URL
		...
		var error := register_http_request.request(url,[],HTTPClient.METHOD_POST, body)
		if error != OK:
			push_error("error occurred while http request", error)
	else:
...
func _on_verify_button_pressed() -> void:
	...
	if codes.size() != 6:
		...
		for character in codes:
			code += character
		var url = "http://"+Constants.HOST+Constants.PORT_WITH_COLON+Constants.REGISTER_VERIFY_URL+self.register_verify_url
		var body = "\"" + code + "\""
		var error = register_verify_http_request.request(url,[],HTTPClient.METHOD_POST, body)
		if error != OK:
			push_error("error occurred while http request", error)
...
func _on_register_verify_http_request_request_completed(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	func(response):
		...
		login_form.visible=true
		login_error_label.modulate=Color(1,1,1,1)
		login_error_label.text="register completed",
	func(msg:String):
		register_verify_error_label.modulate=Color(0.5,0,0,1)
